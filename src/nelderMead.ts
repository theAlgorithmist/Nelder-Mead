/**
 * Nelder-Mead simplex optimization for a function with a modest (say less than ten) number of independent variables.  This
 * implementation is oriented towards applications where the cost of the objective well exceeds the cost of a single
 * (non-shrink) iteration (i.e. statistical parameter estimation).  Some performace optimizations are noted that could be
 * implemented for other use cases.  Two methods are provided for creating the initial simplex and two termination
 * (iteration count and function-value) criteria are supported.  There are known issues with domain-value termination
 * criteria and a reference is provided for a known and highly effective domain convergence test.  Another reference is
 * provided that provides alternate formulas for adaptively setting initial parameter (reflect, expand, contract, shrink)
 * parameters.  This implementation uses initial parameter values that are standard in the literature (and can be
...
export interface NMSimplex
{
  x: Array<number>;
  y: number;
}

export const NM_EPSILON: number = 1.0e-7;
...
): Array<number>
{
  ...
  let iter: number        = 0;
  let reflectedY: number  = 0;
  let expandedY: number   = 0;
  let contractedY: number = 0;
  ...
  for (iter = 0; iter < iterCount; ++iter)
  {
    ...
    if (iter % 2 === 0)
    {
      let sumFcn: number = 0.0;
      for (i = 0; i <= n; ++i) {
        sumFcn += simplex[i].y;
      }

      let avg: number = sumFcn / (n + 1);
      let sum: number = 0.0;
      for (j = 0; j <= n; j++)
      {
